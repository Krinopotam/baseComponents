...
export interface IRuleCustom {
    ...
    callback: (value: any, values: Record<string, unknown> | undefined) => string;
}

export type RuleType = IRuleString | IRuleNumber | IRuleDate | IRuleFile | IRuleObject | IRuleCustom;

export function validate(
    values: Record<string, unknown>,
    validationRules?: Record<string, RuleType[]>,
    ...
    for (const name in validationRules) {
        ...
        const errorMsg = validateValue(val, validationRules[name], values);
...
export function validateValue(val: unknown, validationRules: RuleType[], values?: Record<string, unknown>) {
    for (const rule of validationRules) {
        if (rule.type === 'string') {
            if (val && typeof val === 'object') return 'Неподходящий тип данных';
            if (!validateString(<string>val, rule)) return rule.message;
        } else if (rule.type === 'file') {
            if (val && typeof val === 'object') return 'Неподходящий тип данных';
            if (!validateFile(<string>val, rule)) return rule.message;
        } else if (rule.type === 'number') {
            if (val && typeof val === 'object') return 'Неподходящий тип данных';
            if (!validateNumber(<string>val, rule)) return rule.message;
        } else if (rule.type === 'date') {
            if (!validateDate(<string>val, rule)) return rule.message;
        } else if (rule.type === 'object') {
            if (typeof val !== 'object') return 'Неподходящий тип данных';
            if (!validateRowObject(<{id: string} | [{id: string}]>val, rule)) return rule.message;
        } else if (rule.type === 'custom') {
            if (typeof rule.callback === 'function') return rule.callback(val, values);
        }
    }
...
function validateString(val: string | null, rule: IRuleString) {
...
function validateFile(val: string | null, rule: IRuleFile) {
    const ruleParameter = typeof rule.parameter === 'function' ? rule.parameter(val) : rule.parameter;
    if (val) {
        if (rule.rule === 'not-empty') return val.length > 0;
...
function validateNumber(val: string | null, rule: IRuleNumber) {
    if (!val) {
        val = '';
        if (rule.rule !== 'not-empty') return true;
    }

    if (rule.rule === 'not-empty') return isNumeric(val);
    if (rule.rule === 'number-or-empty') return !val || isNumeric(val);
...
function validateDate(val: string | null, rule: IRuleDate) {
    if (!val) {
        val = '';
        if (rule.rule !== 'not-empty') return true;
    }
...
function validateRowObject(val: {id: string} | [{id: string}], rule: IRuleObject) {
